 ...
 * @memberOf {TypeName} 
 */
function Node(id, name, type, pid,tag,url,objData,searchperson,hc,open,icon, iconOpen ){
	this.id = id;
	this.pid = pid;
	this.name = name;
...
dTree.prototype.add = function (id, name, type, pid,tag,url,objData,searchperson,hc,open,icon, iconOpen) {
	this.aNodes[this.aNodes.length] = new Node(id, name, type, pid,tag,url,objData,searchperson,hc,open,icon, iconOpen);
};
...
dTree.prototype.addNode = function (pNode) {
	...
	for (n; n < this.aNodes.length; n++) {
		if (this.aNodes[n].pid == pNode.id) {
			...
			cn._ai = n;
			this.setCS(cn);
			if (!cn.tag && this.config.tag) {
				cn.tag = this.config.tag;
			}
...
dTree.prototype.node = function (node, nodeId) {
	...
	if (node.url) {
		str += "<a id=\"a" + this.obj + nodeId + "\" style=\"cursor: pointer;\" class=\"" + ((this.config.useSelection) ? ((node._is ? "nodeSel" : "node")) : "node") + "\" ";
		...
		if (this.config.useSelection && ((node._hc && this.config.folderLinks) || !node._hc)) {
			if (node.url == "dbjs") {
			   //双击事件
				str += "  href='#' ondblclick=\"javascript:dbdoJs('" + node.id + "','" + node.objData+ "','" + node.pid + "','"+node.searchperson+"')\"";
			} 
			else if(node.url == "bjs")
			{
			   //单击事件
			   str += "  href='#' onclick=\"javascript:doJs('" + node.id + "','" + node.objData + "','" + node.pid + "','"+node.searchperson+"')\"";
			}
			else {
				if (node.url != "#") {
					str += "href=\"" + node.url + "\" ";
		...
		if ((!this.config.folderLinks || !node.url) && node._hc && node.pid != this.root.id) {
			str += "<a onclick=\"javascript: " + this.obj + ".o(" + nodeId + ");\" href=\"#\" class=\"node\">";
		}
	}
	str += node.name;
...
dTree.prototype.indent = function (node, nodeId) {
	...
	if (this.root.id != node.pid) {
		...
		if (node._hc) {
		    ...
		    if(node.tag==0){
			  str += "<a onclick=\"javascript:getChild('" + node.id + "','" + node.type + "','" + nodeId + "','"+this.aIndent.toString()+"',this);\" href=\"#\" >";
			  str += "<input type='hidden' id=\"hide_"+node.id+"\" value=\""+node.id+"**"+node.type+"**"+nodeId +"\" />";
			  node._io=false;
			}else{
			  str += "<a onclick=\"javascript: " + this.obj + ".o(" + nodeId + ");\" href=\"#\" >";
			}
