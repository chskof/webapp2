 ...
 */
function initUpControl(path,modcode,applyid,userID,userName,status,showclass,showstatus)
{
	 $('#upFile').uploadify({
	        ...
	        'fileTypeDesc' : '支持的上传格式：',//如果配置了以下的'fileExt'属性，那么这个属性是必须的 
...
*
*/
function upfilebylayui(upload,modcode,appid,userName,userID,status,showclass,showstatus){
	var uploadInst = upload.render({
	  elem: '#upfilebtn' //绑定元素
	  ,field:'upFile'
	  ,url: 'cmuc2/fileup.jsp' //上传接口
	  ,size:'20480' //20m
	  ,choose:function(obj){timerint=setInterval("mytimer('正在上传，请稍候')",1000);}	  
	  ,accept:'file'//文件限制类型，可选值有：images（图片）、file（所有文件）、video（视频）、audio（音频）	  
	  ,data: {dotype:'uploadfile',applyId:appid,userName:userName,userID:userID,modcode:modcode,status:status}
	  ,choose: function(obj){
...
function pldown(attids,modcode){
	//从列表重新获取附件id信息
	var trobj= document.getElementsByName("atttrname");
	var attids='';
