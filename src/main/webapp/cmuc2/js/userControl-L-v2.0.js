...
function btnOk() {
	...
	if('TEXTAREA'==$(_pwindos.document).find('#'+_nameinputid)[0].tagName){
		$(_pwindos.document).find('#'+_nameinputid).html(nameList.toString());
	}else{
		$(_pwindos.document).find('#'+_nameinputid).attr('value',nameList.toString());//不用val写入值，保障复制时的一致性
	}
